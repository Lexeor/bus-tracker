...
const LanguageSwitch: FC = () => {
  ...
  const handleLanguageClick = (code: string) => {
    ...
    activateLocale(code).then(() => {
      ...
      toast(i18n._('languageSwitched'), {
        icon: <lang.Flag title={lang.title} className={`w-5 h-5 rounded-sm border flex-shrink-0 ${isDark ? 'border-white/10' : 'border-neutral-200'}`} />,
      });
    });
  };
  ...
  const renderFlag = (code: string) => {
    const lang = LANGUAGES.find((l) => l.code === code)!;
    return <lang.Flag title={lang.title} className={`w-full h-full rounded-sm border-2 ${isDark ? 'border-white/10' : 'border-neutral-200'}`} />;
  };
        ...
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              className={`flex flex-col gap-2 border backdrop-blur-sm rounded-lg shadow-lg p-2 ${isDark ? 'bg-black/40 border-white/10' : 'bg-white/40 border-white/40'}`}
            >
